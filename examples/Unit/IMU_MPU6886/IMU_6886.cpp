...
void IMU_6886::I2C_Read_NBytes(uint8_t driver_Addr, uint8_t start_Addr,
                               uint8_t number_Bytes, uint8_t* read_Buffer) {
    M5.I2C.readBytes(driver_Addr, start_Addr, number_Bytes, read_Buffer);
}

void IMU_6886::I2C_Write_NBytes(uint8_t driver_Addr, uint8_t start_Addr,
...
void IMU_6886::ReadFIFOBuff(uint8_t* DataBuff, uint16_t Length) {
    ...
    for (uint8_t i = 0; i < number; i++) {
        I2C_Read_NBytes(IMU_6886_ADDRESS, IMU_6886_FIFO_R_W, 210,
                        &DataBuff[i * 210]);
    }

    I2C_Read_NBytes(IMU_6886_ADDRESS, IMU_6886_FIFO_R_W, Length % 210,
                    &DataBuff[number * 210]);
}
